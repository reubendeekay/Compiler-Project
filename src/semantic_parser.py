...
class values:
    def __init__(
        self, type=None, width=0, offset=None, args=None, place=None, args_width=None
    ):
        self.type = type
        self.offset = offset
        self.width = width
...
# Generates new temporary variable name(eg var_1, var_2 etc)
def address_generate_compilername(
    offset1=None, width=None, label=None, isf=0, isreg=-1
):
    global addr_compiler_count
    addr_compiler_count += 1
    name = "var_" + str(addr_compiler_count)
    cur_activation[-1].data[name] = {
        "width": width,
        "label": label,
        "func_offset": offset1,
        "isf": isf,
...
def generate_label():
    global lab_count
    lab_count += 1
    return f"lab_{lab_count}"


def generate_forname():
    global for_count
    for_count += 1
    return f"for_{for_count}"
...
def dfs(a, lcounter):
    ...
    if a is not None:
        string = ""
        string = (
            string + "a" + str(lcounter) + ' [label="' + str(a.leaf["label"]) + '"];\n'
        )
...
def p_Import(p):
    ...
    if len(p) == 3:
        ...
        p[0] = Node(
            "void",
            [Node("void", [], {"label": ""}), Node("void", [], {"label": p[1][1:-1]})],
            {"label": "Import"},
        )
...
def p_RepeatTopLevelDecl(p):
    ...
    if len(p) == 2:
        if p[1]:
            ...
            p[0] = Node(
                "void", [], {"label": "empty", "code1": [], "code2": [], "place": None}
            )
    if len(p) == 4:
...
def p_StatementList(p):
    ...
    if len(p) == 2:
        if p[1]:
            ...
            p[0] = Node(
                "void", [], {"label": "StatementList", "code": [], "place": None}
            )
    if len(p) == 4:
        p[3].children = [p[1]] + p[3].children
...
def p_ConstSpec(p):
    ...
    if len(p) == 3:
        ...
        for child in p[1].children:
            ...
            if t is None:
                ...
                if type1[0] == 2 or (type1[0] == 3 and len(type1) % 2 == 0):
                    ...
                    tmp_name = address_generate_compilername(
                        func_offset[-1],
                        p[2].children[0].leaf["width"],
                        cur_activation[-1].label,
                        isf,
                    )
                    func_offset[-1] += p[2].children[0].leaf["width"]
                    p[0].leaf["code"] = []
        ...
        for ind in range(0, len1):
            t = lookup(
                cur_symtab[len(cur_symtab) - 1], p[1].children[ind].leaf["label"]
            )
            if t is None:
                width = p[4].children[ind].leaf["width"]
                type1 = first_nontypedef(p[2].children[0].leaf["type"], cur_symtab[-1])
                type2 = first_nontypedef(
                    p[4].children[ind].leaf["type"], cur_symtab[-1]
                )
                if check_type(type1, type2, cur_symtab[-1]) == False:
                    if len(type1) != 1 or len(type2) != 1:
                        ...
                        exit()
                    elif (type1[0] >= 3 and type1[0] <= 12) and (
                        type2[0] >= 13 and type2[0] <= 14
                    ):
                        print(
                            "[line:"
                            + str(p.lineno(1))
                        ...
                        (type1[0] >= 3 and type1[0] <= 14)
                        and (type2[0] >= 3 and type2[0] <= 14)
                    ):
                        print(
                            "[line:"
                            + str(p.lineno(1))
                ...
                print(
                    (
                        ...
                        + "]"
                        + "Redeclaration of "
                        + str(p[1].children[ind].leaf["label"])
                        + " at line "
                        + str(p.lineno(1))
                    )
                )
...
def p_TypeDef(p):
    ...
    TypeDef : ID K Types
    """
    p[0] = Node("void", [Node("void", [], {"label": p[1]}), p[3]], {"label": "TypeDef"})
...
def p_VarSpec(p):
    ...
    if len(p) == 3:
        ...
        p[0] = Node(
            "void", [p[1], Node("void", [], {"label": "="}), p[5]], {"label": "VarSpec"}
        )
        len1 = len(p[1].children)
        ...
        for ind in range(0, len1):
            ...
            if t is None:
                ...
                type1 = first_nontypedef(p[2].children[0].leaf["type"], cur_symtab[-1])
                type2 = first_nontypedef(
                    p[5].children[ind].leaf["type"], cur_symtab[-1]
                )
                if check_type(type1, type2, cur_symtab[-1]) == False:
                    if len(type1) != 1 or len(type2) != 1:
                        print(
...
def p_FunctionMarker(p):
    ...
    if t is None:
        ...
        if t.args is None:
            ...
            if check_type(
                t.type, p[0].children[3].leaf["type"], cur_symtab[-2]
            ) == True and len(t.args) == len(p[0].children[2].leaf["type"]):
                for x in range(0, len(t.args)):
                    if (
                        check_type(
                            t.args[x], p[0].children[2].leaf["type"][x], cur_symtab[-2]
                        )
                        == 0
                    ):
                        print(
                            ...
                            + str(p.lineno(1))
                            + "]"
                            + "Function does not align with definition at line "
                            + str(p.lineno(1))
                        )
                        exit()
            else:
...
def p_Signature(p):
    ...
    if len(p) == 2:
        p[0] = Node(
            "void",
            [p[1], Node("void", [], {"label": "Result", "type": [], "width": 0})],
            {"label": "Signature"},
        )
    else:
    ...
    for i in reversed(range(0, len(p[1].leaf["list"]))):
        ...
        cur_activation[-1].data[cur_symtab[-1].data[p[1].leaf["list"][i]].place][
            "func_offset"
        ] = running_offset
...
def p_Parameters(p):
    ...
    if len(p) == 4:
        p[0] = Node(
            "void",
            [Node("void", [], {"label": "("}), Node("void", [], {"label": ")"})],
            {"label": "Arguments", "type": [], "width": [0], "list": []},
        )
    else:
        p[0] = Node(
            "void",
    ...
    if len(p) == 3:
        ...
        if t is None:
            ...
            p[0] = Node(
                ...
                {"label": p[1], "type": p[2].children[0].leaf["type"], "width": wi},
            )
        else:
            print(
                "[line:"
...
def p_LabeledStmt(p):
    ...
    if p[1].children[0].leaf["label"] in cur_symtab[-1].label_map:
        ...
        p[0].leaf["code"] = [["label", p[1].children[0].leaf["label"]]] + p[4].leaf[
            "code"
        ]
        p[0].leaf["place"] = None
...
def p_ExpressionStmt(p):
    ...
    if "statement" not in p[1].leaf:
        print(
            "Only function Callings can be Expression Statements.Error at lineno "
            + str(p.lineno(1))
        )
        exit()
...
def p_IncDecStmt(p):
    ...
    """
    p[0] = Node(
        "void", [p[1]] + [Node("void", [], {"label": p[2]})], {"label": "IncDecStmt"}
    )
    if "marked" not in p[1].children[0].leaf:
        print(
            "Increment/Decrement allowed only to Identifiers.Error at lineno "
            + str(p.lineno(1))
        )
        exit()
    type1 = p[1].leaf["type"]
    ...
    if len(p) == 4:
        if p[3] == "null":
            ...
            if "marked" not in p[1].children[0].children[0].leaf:
                print(
                    "Assignment allowed only to Identifiers.Error at lineno "
                    + str(p.lineno(1))
                )
                exit()
            ...
            p[0].leaf["code"].append(
                [
                    "openFile",
                    p[1].children[0].leaf["place"],
                    p[3].children[0].leaf["place"],
                    p[3].children[1].leaf["place"],
                    p[3].children[2].leaf["place"],
                ]
            )
        ...
        if p[1].leaf["label"] == "readFile":
            ...
            p[0].leaf["code"].append(
                [
                    "readFile",
                    p[1].children[0].leaf["place"],
                    p[1].children[1].leaf["place"],
                    p[1].children[2].leaf["place"],
                ]
            )
        ...
        if p[2] == "=":
            ...
            for ind in range(0, len1):
                type1 = first_nontypedef(
                    p[1].children[ind].leaf["type"], cur_symtab[-1]
                )
                type2 = first_nontypedef(
                    p[4].children[ind].leaf["type"], cur_symtab[-1]
                )
                ...
                p[0].leaf["code"] += (
                    p[1].children[ind].leaf["code"] + p[4].children[ind].leaf["code"]
                )
                if type1[0] > 12 and type1[0] <= 14 and type2[0] <= 12:
                    t2 = address_generate_compilername(
                        func_offset[-1], 4, cur_activation[-1].label, 1
            ...
            if p[2].children[0].leaf["label"] in ["&=", "^=", "|=", ">>=", "<<=", "%="]:
                for ind in range(0, len1):
                    ...
                    if len(type1) != 1 or len(type2) != 1:
                        ...
                        exit()
                    if not (
                        (type1[0] >= 3 and type1[0] <= 12)
                        and (type2[0] >= 3 and type2[0] <= 12)
                    ):
                        print(
                            "[line:"
                            + str(p.lineno(1))
...
def p_ShortVarDecl(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": p[1]}), Node("void", [], {"label": p[2]}), p[4]],
        {"label": "Assignment"},
    )
...
def p_ReturnStmt(p):
    ...
    if top.label == "global":
        print(
            ...
            + str(p.lineno(1))
            + "]"
            + "Return statement should be inside function",
            p.lineno(1),
        )
        exit()
    ...
    if len(p) == 2:
        if len(ret_type) != 0:
            print(
                ...
                + str(p.lineno(1))
                + "]"
                + "No return expression given but expected"
            )
            exit()
        p[0] = Node(
            "void", [Node("void", [], {"label": "return"})], {"label": "ReturnStmt"}
        )
        if func == "main":
            p[0].leaf["code"] = [["returnm"]]
        else:
        ...
        if len(p[2].leaf["type"]) != 1:
            print(
                "[line:" + str(p.lineno(1)) + "]" + "Only a single return type allowed"
            )
            exit()
        ...
        p[0] = Node(
            "void",
            [Node("void", [], {"label": "return"}), p[2]],
            {"label": "ReturnStmt"},
        )
        if func == "main":
            p[0].leaf["code"] = p[2].leaf["code"] + [["returnm", p[2].leaf["place"][0]]]
        else:
            p[0].leaf["code"] = p[2].leaf["code"] + [["return", p[2].leaf["place"][0]]]
        p[0].leaf["place"] = None
...
def p_BreakStmt(p):
    ...
    if top.label not in ["for", "case", "default"]:
        print(
            ...
            + str(p.lineno(1))
            + "]"
            + "Break is not inside inside switch or for at ",
            p.lineno(1),
        )
        exit()
    ...
    if len(p) == 2:
        p[0] = Node(
            "void", [Node("void", [], {"label": "break"})], {"label": "BreakStmt"}
        )
...
def p_ContinueStmt(p):
    ...
    if top.label != "for":
        print(
            "[line:" + str(p.lineno(1)) + "]" + "Continue is not inside for loop at ",
            p.lineno(1),
        )

    if len(p) == 2:
        p[0] = Node(
            "void", [Node("void", [], {"label": "continue"})], {"label": "ContinueStmt"}
        )
    ...
    if len(p) == 3:
        ...
        if len(p[1].leaf["code1"]) == 1:
            if len(p[0].leaf["code1"]) == 0:
                ...
                print(
                    "[line:" + str(p.lineno(1)) + "]" + "Multiple Defaults not allowed"
                )
                exit()
...
def p_ExprCaseClause(p):
    ...
    p[0] = Node(
        "void",
        p[1].children + [p[4]],
        {"label": p[-1].leaf["label"], "place": p[-1].leaf["place"]},
    )
...
def p_ExprSwitchCase(p):
    ...
    if len(p) == 4:
        p[0] = Node(
            "void",
            [Node("void", [], {"label": "default"})],
            {"label": p[2].leaf["label"]},
        )
    else:
        p[0] = Node(
            "void",
            [Node("void", [], {"label": "case"}), p[4]],
            {
                "label": p[2].leaf["label"],
                "code": p[4].leaf["code"],
...
def p_ForStmt(p):
    ...
    if len(p) == 5:
        p[0] = Node(
            "void", [Node("void", [], {"label": "for"}), p[3]], {"label": "ForStmt"}
        )
        ...
        p[0] = Node(
            "void",
            [Node("void", [], {"label": "for"}), p[3], p[4]],
            {"label": "ForStmt"},
        )
        if p[4].leaf["label"] == "Condition":
            code = [[p[2].leaf["label"], ":"]] + p[4].leaf["code"]
            code += [
                ["iffalse ", p[4].leaf["place"], " goto ", p[2].leaf["label"] + ".next"]
            ]
...
def p_ForClause(p):
    ...
    if len(p) == 3:
        ...
        if p[2] == ";" and p[4] == ";":
            p[0] = Node(
                "void",
                [p[1], p[3], Node("void", [], {"label": "PostStmt"})],
                {"label": "ForClause"},
            )
            p[0].leaf["place"] = p[3].leaf["place"]
            p[0].leaf["code"] = [p[1].leaf["code"], p[3].leaf["code"], []]
        ...
        p[0].leaf["place"] = p[3].leaf["place"]
        p[0].leaf["code"] = [p[1].leaf["code"], p[3].leaf["code"], p[5].leaf["code"]]
...
def p_PrintIntStmt(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": "printInt"}), p[2]],
        {
            "label": "printInt",
            "code": p[2].leaf["code"] + [["printInt", p[2].leaf["place"]]],
        },
    )
...
def p_PrintFloatStmt(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": "printFloat"}), p[2]],
        {
            "label": "printFloat",
            "code": p[2].leaf["code"] + [["printFloat", p[2].leaf["place"]]],
        },
    )
...
def p_PrintStrStmt(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": "printStr"}), p[2]],
        {
            "label": "printStr",
            "code": p[2].leaf["code"] + [["printStr", p[2].leaf["place"]]],
        },
    )
...
def p_ScanIntStmt(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": "ScanInt"}), p[2]],
        {
            "label": "ScanInt",
            "code": p[2].leaf["code"] + [["ScanInt", p[2].leaf["place"]]],
        },
    )
...
def p_ScanstrStmt(p):
    ...
    p[0] = Node(
        ...
        {
            "label": "ScanInt",
            "code": p[2].leaf["code"] + [["ScanStr", p[2].leaf["place"], name]],
        },
    )
...
def p_Term2(p):
    ...
    if len(p) == 2:
        ...
        p[0] = p[1]
    elif (p[2].children[0].leaf["label"] != "==") and (
        p[2].children[0].leaf["label"] != "!="
    ):
        p[4].leaf["label"] = "Expression"
        p[1].leaf["label"] = "Expression"
        ...
        if p[4] == "null":
            ...
            p[0] = Node(
                ...
                [
                    Node(
                        "void", p[1].children, {"label": p[2].children[0].leaf["label"]}
                    )
                ],
                {"label": "Term2"},
            )
            ...
            if (type1[0] >= 3 and type1[0] <= 12) and (
                type2[0] >= 3 and type2[0] <= 12
            ):
                p[0].leaf["type"], p[0].leaf["width"] = [type_map["int"]], 4
                p[2] = p[2].children[0].leaf["label"] + "int"

            elif (type1[0] >= 3 and type1[0] <= 14) and (
                ...
                p[0].leaf["type"], p[0].leaf["width"] = [16], p[1].leaf["width"] + p[
                    4
                ].leaf["width"]
...
def p_UnaryExp(p):
    ...
    if len(p) == 2:
        ...
        p[0] = Node("void", p[3].children + p[1].children, {"label": "BasicLit"})
        ...
        if p[1].children[0].leaf["label"] in ["+"]:
            ...
            if p[3].leaf["place"][0] == "*":
                ...
                p[3].leaf["place"][-1] == "]" or len(p[3].leaf["place"].split(".")) != 1
            ):
                v1 = address_generate_compilername(
                    func_offset[-1],
                    p[0].children[0].leaf["width"],
        ...
        if p[2].leaf["label"] == "Selector":
            ...
            if len(type_p) == 1:
                print(
                    ...
                    + str(p.lineno(1))
                    + "]"
                    + "Unexpected . with the given input "
                )
                exit()
            elif type_p[0] != 3 and not (
                len(type_p) >= 4 and type_p[0] == 1 and type_p[2] == 5
            ):
                print(type_p)
                print(
                    "[line:"
                ...
                if type_p[0] == 3:
                    if nam not in cur_symtab[-1].struct_name_map[type_p[1]].data:
                        print(
                            "[line:" + str(p.lineno(1)) + "]" + "id not in Structure "
                        )
                        exit()
            ...
            if len(type_p) == 1:
                print(
                    ...
                    + str(p.lineno(1))
                    + "]"
                    + "Unexpected Indexing with the given input "
                )
                exit()
            elif type_p[0] != 2:
                print(
                ...
                p[0].children[0].leaf["width"] = (
                    p[1].children[0].leaf["width"] / type_p[1]
                )
            ...
            if len(p[1].children) != 1:
                print(
                    "[line:" + str(p.lineno(1)) + "]" + "Only function calling allowed "
                )
                exit()
            if type1 is None:
            ...
            if len(type_arg) != len(type1):
                print(
                    "[line:" + str(p.lineno(1)) + "]" + "Mismatch number of Arguments"
                )
                exit()
            for ind in range(0, len(type1)):
                if check_type(type_arg[ind], type1[ind], cur_symtab[0]) == 0:
                    print(
                        ...
                        + str(p.lineno(1))
                        + "]"
                        + "Type Mismatch in function definition"
                    )
                    exit()
            ...
            for i in range(len(type1)):
                if (
                    len(p[2].leaf["place"][i].split(".")) != 1
                    or len(p[2].leaf["place"][i].split("[")) != 1
                ):
                    typ = p[2].leaf["type"][i]
...
def p_stringLit(p):
    ...
    p[0] = Node(
        "void",
        [Node("void", [], {"label": p[1], "type": [type_map["string"]], "width": 4})],
        {"label": "String"},
    )
    ...
    p[0] = Node(
        ...
        [
            Node(
                "void",
                [],
                {"label": p[1], "type": [type_map[p[1]]], "width": type_width[p[1]]},
            )
        ],
        {"label": "Mytypes"},
    )
    ...
    p[0] = Node(
        ...
        [
            Node(
                "void",
                p[6].children,
                {"label": "struct", "type": [3, p[2]], "width": top.total},
            )
        ],
        {"label": "StructType"},
    )
        ...
        [
            Node(
                ...
                + p[5].children,
                {
                    ...
                    "width": int(p[3].children[0].leaf["label"])
...
def p_LiteralValue(p):
    ...
    if len(p) == 4:
        p[0] = Node(
            "void", [Node("void", [], {"label": "empty"})], {"label": "LiteralValue"}
        )
    else:
        ...
        p[0] = Node(
            ...
            [
                Node(
                    "void",
                    [],
                    {"label": p[1], "type": [5, p[1]], "width": val["width"]},
                )
            ],
            {"label": "OperandName"},
        )
    ...
    p[0] = Node(
        ...
        [
            Node(
                "void",
                [],
                {"label": p[1], "type": tmp.type, "width": tmp.width, "marked": 1},
            )
        ],
        {"label": "OperandName2"},
    )
...
def p_IdentifierList(p):
    ...
    if len(p) == 2:
        p[0] = Node(
            "void", [Node("void", [], {"label": p[1]})], {"label": "IdentifierList"}
        )
    else:
